...
import (
	...
	"github.com/scorestack/scorestack/dynamicbeat/checks/ssh_ca"
	"github.com/scorestack/scorestack/dynamicbeat/checks/ssh_domain"
	"github.com/scorestack/scorestack/dynamicbeat/checks/tcp"
...
func RunChecks(defPass chan []schema.CheckConfig, pubQueue chan<- beat.Event) {
	...
	for _, def := range defs {
		...
		if err != nil {
			logp.Warn("Failed to unpack check definition (%+v): %v", def, err)
			continue
...
func unpackDef(config schema.CheckConfig) (schema.Check, error) {
	...
	case "ssh":
		def = &ssh.Definition{}
	case "ssh_ca":
		def = &ssh_ca.Definition{}
	case "ssh_domain":
		def = &ssh_domain.Definition{}
	case "dns":
		def = &dns.Definition{}
	case "ftp":
		def = &ftp.Definition{}
