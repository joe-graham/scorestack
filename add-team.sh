...
ELASTICSEARCH_HOST=elasticsearch4.newscrier.org:9200
KIBANA_HOST=kibana.newscrier.org:5601
CHECK_FOLDER=checks
USERNAME=root
...
do
  ...
  do
    ...
    then
      cat ${CHECK_FOLDER}/${check}/admin-attribs.json | sed -e "s/\${TEAM_NUM}/${TEAM_NUM}/g" > ${CHECK_FOLDER}/${check}/admin-attribs.tmp.json
    ...
    then
      cat ${CHECK_FOLDER}/${check}/user-attribs.json | sed -e "s/\${TEAM_NUM}/${TEAM_NUM}/g" > ${CHECK_FOLDER}/${check}/user-attribs.tmp.json
  ...
  # Add team role
  curl -kX PUT -u ${USERNAME}:${PASSWORD} https://${KIBANA_HOST}/api/security/role/${TEAM} -H 'Content-Type: application/json' -H 'kbn-xsrf: true' -d '{"elasticsearch":{"indices":[{"names":["results-'${TEAM}'*", "checks"],"privileges":["read"]},{"names":["attrib_user_'${TEAM}'"],"privileges":["read","index","view_index_metadata"]}]}, "kibana":[{"base":["read"],"spaces":["scorestack"]}]}'

  # Add team user
  curl -kX PUT -u ${USERNAME}:${PASSWORD} https://${ELASTICSEARCH_HOST}/_security/user/${TEAM} -H 'Content-Type: application/json' -d '{"password":"changeme","roles":["'${TEAM}'"]}'
